...
\begin{equation}
    V(r) = 4\varepsilon((\frac{\sigma}{r})^{12} - (\frac{\sigma}{r})^6), \label{eq:lj}
\end{equation}
...
\noindent
\[
  \frac{d^2\bm{r}_i}{dt^2} = \frac{24\varepsilon}{m} \sum_{j \neq i} (2(\frac{\sigma}{\vert\bm{r}_i-\bm{r}_j}\vert)^{12}-(\frac{\sigma}{\vert\bm{r}_i-\bm{r}_j}\vert)^6)\frac{\bm{r}_i-\bm{r}_j}{\vert\bm{r}_i-\bm{r}_j\vert^2}.
\]
...
smaller than $10^{-25}$kg, while typical length scales are
on the order of nanometers, $10^{-9}$m.
...
\noindent
\begin{equation}
\frac{d^2\bm{r}_i\,'}{{dt'^2}} = 24 \sum_{j \neq i} (2\vert\bm{r}_i\,'-\bm{r}_j\,'\vert^{-12}-\vert\bm{r}_i\,'-\bm{r}_j\,\vert^{-6})\frac{\bm{r}_i\,'-\bm{r}_j\,'}{\vert\bm{r}_i\,'-\bm{r}_j\,'\vert^2}, \label{eq:undim}
\end{equation}
\begin{itemize}
\item \textbf{1k (5pt):} What is the characteristic time scale $\tau$, and what is its value for argon, which has $\sigma=3.405$Å ($1 \mathrm{Å}=1e-10$m), $m = 39.95u$ with ($1u=1.66e-27$kg) and $\varepsilon=1.0318e-2$eV 1eV=$1.602e-19$J)?
\end{itemize}
...
\item \textbf{1p (5pt):} Repeat the previous tasks, but now with an initial separation of $0.95\sigma$. Explain your results.
